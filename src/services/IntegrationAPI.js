...
export async function connectToUAEBank(bankCode, credentials) {
...
export async function syncBankTransactions(connectionId, dateRange) {
...
export function exportToAccountingSoftware(data, format = "csv") {
    switch (format) {
        case "json":
            return JSON.stringify(data, null, 2);
        case "xml":
            // Simple XML conversion
            const xmlData = data.map(item => `<transaction><date>${item.date}</date><amount>${item.amount}</amount><type>${item.type}</type><description>${item.description}</description></transaction>`).join('\n');
